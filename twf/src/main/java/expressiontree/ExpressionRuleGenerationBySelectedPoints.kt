...
fun generateParentBracketsExpansionSubstitution(substitutionSelectionData: SubstitutionSelectionData): List<SubstitutionApplication> {
    val result = mutableListOf<SubstitutionApplication>()
    if (substitutionSelectionData.compiledConfiguration.expressionTreeAutogeneratedTransformationRuledentifiers.containsKey(PARENT_BRACKETS_EXPANSION)) {
        val subst = substitutionSelectionData.compiledConfiguration.expressionTreeAutogeneratedTransformationRuledentifiers[PARENT_BRACKETS_EXPANSION]!!
...
fun generatePermutationSubstitutions(substitutionSelectionData: SubstitutionSelectionData,
    ...
    if (substitutionSelectionData.selectedSubtreeTopArgumentsInSelectionOrder != null && substitutionSelectionData.selectedSubtreeTopArguments?.functionStringDefinition?.function?.isCommutativeWithNullWeight == true &&
        ...
        if (substitutionSelectionData.selectedNodes.size == 2 && userSelectedDirectlyChildrenOfCommutativeNode && substitutionSelectionData.compiledConfiguration.expressionTreeAutogeneratedTransformationRuledentifiers.containsKey("ArgumentsSwap")) {
        ...
        if (substitutionSelectionData.compiledConfiguration.expressionTreeAutogeneratedTransformationRuledentifiers.containsKey("ArgumentsPermutation")) {
            ...
            if (!swapSubstitutionSuggested && substitutionSelectionData.notSelectedSubtreeTopArguments != null && substitutionSelectionData.notSelectedSubtreeTopArguments!!.children.size == 1 &&
                ...
                if (substitutionSelectionData.selectedSubtreeTopArgumentsInSelectionOrder!!.toString() != substitutionSelectionData.selectedSubtreeTopArguments!!.toString() &&
                        ...
                        substitutionSelectionData.compiledConfiguration.expressionTreeAutogeneratedTransformationRuledentifiers.containsKey("ArgumentsPermutationInOriginalOrder")) {
                    subst = substitutionSelectionData.compiledConfiguration.expressionTreeAutogeneratedTransformationRuledentifiers["ArgumentsPermutationInOriginalOrder"]!!
...
fun generateReduceArithmeticSubstitutions(substitutionSelectionData: SubstitutionSelectionData,
    ...
    if (substitutionSelectionData.selectedSubtreeTopArgumentsInSelectionOrder != null && substitutionSelectionData.selectedSubtreeTopArgumentsInSelectionOrder!!.children.size > 1) {
        ...
        if (dotOperation.isNotBlank() && substitutionSelectionData.compiledConfiguration.expressionTreeAutogeneratedTransformationRuledentifiers.containsKey("TwoSidesArithmeticReduce")) { // + (*) -> * (+)
...
private fun tryToAddMonomReduceTransformation(plusOperation: String, dotOperation: String, substitutionSelectionData: SubstitutionSelectionData, useExpanded: Boolean, simplifyNotSelectedTopArguments: Boolean, result: MutableList<SubstitutionApplication>,
    ...
    val subst = substitutionSelectionData.compiledConfiguration.expressionTreeAutogeneratedTransformationRuledentifiers["TwoSidesArithmeticReduce"] ?: return emptyList()
...
fun generateReduceFractionSubstitutions(substitutionSelectionData: SubstitutionSelectionData,
                                        ...
                                        alreadyAddedSubstitutionCodes: Set<String> = setOf()): List<SubstitutionApplication> {
    val result = mutableListOf<SubstitutionApplication>()
    if (substitutionSelectionData.compiledConfiguration.expressionTreeAutogeneratedTransformationRuledentifiers.containsKey(REDUCE_FRACTION) &&
            REDUCE_FRACTION !in alreadyAddedSubstitutionCodes) {
        val subst = substitutionSelectionData.compiledConfiguration.expressionTreeAutogeneratedTransformationRuledentifiers[REDUCE_FRACTION]!!
...
fun generateComplicatingExtensionSubstitutions(substitutionSelectionData: SubstitutionSelectionData,
    ...
    if (substitutionSelectionData.selectedNodes.size == 2) {
        ...
        if (substitutionSelectionData.compiledConfiguration.expressionTreeAutogeneratedTransformationRuledentifiers.containsKey("MultiplicativeComplicatingExtension") &&
                !substitutionSelectionData.compiledConfiguration.factComporator.expressionComporator.baseOperationsDefinitions.additivelyEqual(applicationObject.computeNodeIfPossible() ?: 1.0, 0.0)) //check to avoid divisions on zero. Current verification is only fast appropriate solution, todo: it should be fixed, otherwise it might be possiible to find cheat solutions for some tasks
        {
