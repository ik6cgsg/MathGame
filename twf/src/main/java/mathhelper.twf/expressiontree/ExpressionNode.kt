            ...
            when (functionIdentifier.defaultStringDefinitionType) {
                StringDefinitionType.BINARY_OPERATION -> {
                    ...
                    if (children.size > 0) {
                        ...
                        for (i in 1..children.lastIndex) {
                            ...
                            identifier += children[i].toPlainTextViewRec(functionConfiguration)
                        }
                    }
                    if (binaryOperationNeedBrackets()) {
                        identifier += ")"
                ...
                StringDefinitionType.UNARY_LEFT_OPERATION -> {
                    if (functionConfiguration.functionProperties.find { it.function == parent?.value }?.defaultStringDefinitionType in listOf(StringDefinitionType.FUNCTION, StringDefinitionType.BINARY_OPERATION)) {
                        identifier += functionIdentifier.plainTextRepresentation + children.first().toPlainTextViewRec(functionConfiguration)
                    } else {
                        identifier += "(" + functionIdentifier.plainTextRepresentation + children.first().toPlainTextViewRec(functionConfiguration) + ")"
                    }
                }
