...
import com.twf.standartlibextensions.findClosestPlaceToTargetOnTheSameLevel
import com.twf.standartlibextensions.findFirstNotInTagNotInMtext
import com.twf.standartlibextensions.skipClosingTags
import com.twf.standartlibextensions.skipFromRemainingExpressionWhileClosingTagNotFound
...
fun dropPerformedBrushing (string: String): String {
    var result = dropPerformedBrushingInternal(string)
    var resultLast = string
    while (result != resultLast){
        val newResult = dropPerformedBrushingInternal(result)
        resultLast = result
        result = newResult
    }
    return result
...
fun dropPerformedBrushingInternal (string: String): String {
    val result = StringBuilder()
