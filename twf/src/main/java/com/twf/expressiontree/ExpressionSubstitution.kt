...
class VarNamesTimeStorage (var time: Int = 0) {
    ...
    fun addVarName (name: String, type: SubstitutionInstanceVarType) = addVarName(SubstitutionInstanceVar(name, type, time))
    ...
    fun popVarsAfter (time: Int): List<SubstitutionInstanceVar> { // inclusive
        val parts = varsList.partition { it.time >= time } // can be done more effective, by O(log(n)) - just find start be binary search and split list. todo: fix it
...
data class SubstitutionInstance(
    ...
    fun logValue (message: () -> String, messageType: MessageType = MessageType.TECHNICAL, level: Int){
        log.add(isApplicable, correspondingIndexes.matchedFactIndex, {"${message.invoke()}, isApplicable: '"}, {"', matchedFactIndex: '"}, {"'"})
        log.add(varRuleNameToExpressionValueMap.entries.joinToString (separator = ", ") { "${it.key} = '${it.value.toString()}'" }, {"NameToExpressionValue: '''"}, {"'''"},
                messageType = messageType, level = level + 1)
        log.add(varRuleFunctionNameArgsToExpressionMap.entries.joinToString (separator = ", "){ "${it.key} = '${it.value.toString()}'" }, {"FunctionNameArgsToExpression: '''"}, {"'''"},
                messageType = messageType, level = level + 1)
        log.add(varRuleNameToComparableTransformationsPartMap.entries.joinToString (separator = ", "){ "${it.key} = '${it.value.toString()}'" }, {"NameToFact: '''"}, {"'''"},
                messageType = messageType, level = level + 1)
    }
    ...
    fun dropExtraVarsAfter (time: Int){
        ...
        for (extraVar in extraVars){
            when (extraVar.type){
                ...
                SubstitutionInstanceVarType.COMPARABLE_VAR -> varRuleNameToComparableTransformationsPartMap.remove(extraVar.name)
            }
        }
    }
...
class ExpressionSubstitution(
    ...
    fun checkCondition(expressionNode: ExpressionNode, conditionNode: ExpressionNode,
                       substitutionInstance: SubstitutionInstance,
                       nameArgsMap: MutableMap<String, String> = mutableMapOf()) {
        checkConditionCompanion(expressionNode, conditionNode, substitutionInstance, nameArgsMap, basedOnTaskContext)
    }

    companion object {
        fun checkConditionCompanion(expressionNode: ExpressionNode, conditionNode: ExpressionNode,
                           substitutionInstance: SubstitutionInstance,
                           nameArgsMap: MutableMap<String, String> = mutableMapOf(),
            ...
            if (conditionNode.isNumberValue()) {
                ...
                if (!basedOnTaskContext && conditionNode.functionStringDefinition != null &&
                    ...
                    val argumentStartIndex = conditionNode.functionStringDefinition?.function?.numberOfDefinitionArguments ?: 0
                    ...
                    for (i in argumentStartIndex..conditionNode.children.lastIndex) {
                        checkConditionCompanion(expressionNode.children[i], conditionNode.children[i], substitutionInstance, nameArgsMap, basedOnTaskContext)
            ...
            if (right.value.isEmpty() && right.children.size == 1) {
                ...
                if (right.functionStringDefinition!!.function.isNameForRuleDesignations) {
                    ...
                    for (j in 0..right.children.lastIndex)
                        functionNameArgsToExpressionMap.put("sys_def_func_agr_$j",
                                substitutionInstance.getExprVar(right.children[j].value)?.cloneWithNormalization(sorted = false) ?: return null)
                    return substitutionInstance.getExprFunc(right.value + "_" + right.children.size)?.
                            cloneWithDeepSubstitutions(functionNameArgsToExpressionMap)?.normalizeSubTree(sorted = false) ?: return null
                } else {
    ...
    private fun findAllPossibleSubstitutionPlaces (root: ExpressionNode, result: MutableList<SubstitutionPlace>) {
        for (i in 0 until root.children.size){
            findAllPossibleSubstitutionPlaces(root.children[i], result)
            val substitutionInstance = checkLeftCondition(root.children[i])
            if (substitutionInstance.isApplicable){
                result.add(SubstitutionPlace(root, i, substitutionInstance, root.children[i]))
            }
        }
    }
    ...
    fun findAllPossibleSubstitutionPlaces (root: ExpressionNode): MutableList<SubstitutionPlace>{
        ...
        findAllPossibleSubstitutionPlaces(root, result)
        return result
    }
    ...
    fun applySubstitutionByBitMask (substitutionPlaces: List<SubstitutionPlace>, bitMask: Int){
        for (i in 0 until substitutionPlaces.size) {
            ...
            substitutionPlace.nodeParent.children[substitutionPlace.nodeChildIndex] = substitutionPlace.originalValue //roll back previous substitutions
        }
        for (i in 0 until substitutionPlaces.size){
            val substitutionPlace = substitutionPlaces[i]
            ...
            if (newValue != null) {
                substitutionPlace.nodeParent.children[substitutionPlace.nodeChildIndex] = newValue //to save. to test such case for expressions - todo - done
            }
        }
    }

    fun applyRight(substitutionInstance: SubstitutionInstance, right: ExpressionNode = this.right.children[0]) = applyRightCompanion(substitutionInstance, right)
